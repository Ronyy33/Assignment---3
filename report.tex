...
\begin{itemize}
    \item \textbf{JackTokenizer.py}: Strips comments using a character-by-character state machine and tokenizes the source into keywords, symbols, integer constants, string constants, and identifiers. Defines the \texttt{JackLexicalLexer} class. Writes the token XML file (\texttt{<Class>T.xml}).
    \item \textbf{CompilationEngine.py}: Recursive-descent parser implementing every non-terminal in the Jack grammar. Defines the \texttt{SyntaxAnalyzer} class. Simultaneously writes the parse tree XML (\texttt{<Class>.xml}) and emits VM code through the \texttt{BytecodeWriter}.
    \item \textbf{SymbolTable.py}: Two-scope symbol table (class-level and subroutine-level) defining the \texttt{LexicalScopeTable} class mapping identifiers to \texttt{(type, kind, index)}.
    \item \textbf{VMWriter.py}: Thin wrapper that writes VM commands via the \texttt{BytecodeWriter} class to the output file.
...
The key design decision was to have the \texttt{SyntaxAnalyzer} do both XML generation and VM code generation in a single pass. This means every \texttt{parse\_*} method writes XML tags while also emitting VM instructions via the \texttt{BytecodeWriter}. An alternative would have been two separate passes, but the single-pass approach keeps the code simpler and avoids having to maintain a separate AST data structure.
...
The five XML-unsafe symbols (\texttt{<}, \texttt{>}, \texttt{\&}, \texttt{"}) are escaped in the XML output. One bug I hit early on was escaping \texttt{\&} \emph{after} \texttt{<}, which turned \texttt{\&lt;} into \texttt{\&amp;lt;}. The fix was to always escape \texttt{\&} first in the XML escape helper function.

% ============================================================
\section{Parser Design}
% ============================================================

The parser implements recursive descent following the Jack grammar from Figure 10.5 of the textbook. Each grammar non-terminal has a corresponding \texttt{parse\_*} method.
...
\begin{itemize}
    \item \texttt{\_require\_token(type, value)}: Consumes the current token, writes it to XML, asserts it matches the expected type/value, and advances. This is the workhorse --- almost every terminal in the grammar goes through \texttt{\_require\_token}.
    \item \texttt{\_matches\_token(type, value)}: Peeks at the current token without consuming. Used for lookahead decisions (e.g., is the next statement a \texttt{let} or a \texttt{while}?).
    \item \texttt{\_lookahead\_next\_token()}: Looks one token ahead. Only needed for the LL(2) case in \texttt{parse\_term}.
\end{itemize}

\subsection{The LL(2) Problem in \texttt{parse\_term}}
...
\begin{lstlisting}[style=pseudo, caption={Expression parsing (left-to-right)}]
parse_expression:
    parse_term
...
At the point where the inner-loop accumulator is assigned (\texttt{let cellVal = cellVal + ...}), the symbol tables look like this:
...
\midrule
kernelMatrix & Array & static & 0 \\
strideStep & int & static & 1 \\
inputTensor & Array & this & 0 \\
outputTensor & Array & this & 1 \\
inputSize & int & this & 2 \\
\bottomrule
\end{tabular}
\end{table}
...
So \texttt{cellVal} resolves to \texttt{local 7}, \texttt{inputTensor} to \texttt{this 0}, \texttt{inputSize} to \texttt{this 2}, \texttt{kernelMatrix} to \texttt{static 0}, and \texttt{strideStep} to \texttt{static 1}. The implicit \texttt{this} at argument 0 is pushed and popped into \texttt{pointer 0} at the start of the method.
...
For \texttt{Conv.new}, this allocates 3 words (for \texttt{inputTensor}, \texttt{outputTensor}, \texttt{inputSize}).
...
When \emph{calling} a method (e.g., \texttt{convolutionEngine.initFilter(filterKernel)}), the compiler pushes the object reference as the first argument before the explicit arguments, and increments the argument count by 1.
...
\subsection{Compilation Correctness: Tracing \texttt{let outputTensor[outIdx] = cellVal}}

For the 5$\times$5 test case, consider the first output assignment where $outIdx = 0$ and $cellVal = 7$. The emitted VM code is:

\begin{lstlisting}[style=hackvm, caption={VM trace for let outputTensor[outIdx] = cellVal}]
push this 1       // outputTensor base address (say, addr 2050)
push local 8      // outIdx = 0
add               // 2050 + 0 = 2050
push local 7      // cellVal = 7
pop temp 0        // temp 0 = 7
...
So \texttt{THAT} is set to the base address of \texttt{outputTensor} plus offset $outIdx$, and the computed cellVal is stored there via \texttt{that 0}. This correctly writes 7 to the first element of the output array.
...
\begin{itemize}
    ...
    \item \textbf{Static vs. field}: The filter kernel (\texttt{kernelMatrix}) and stride step (\texttt{strideStep}) are \texttt{static} (shared across all Conv instances), while the input/output tensors (\texttt{inputTensor}, \texttt{outputTensor}) and size (\texttt{inputSize}) are \texttt{field} (per-instance).
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Tokenizer}: Compiled \texttt{Conv.jack} and checked that \texttt{ConvT.xml} had the right tokens and that all comments were stripped. Verified XML escaping of \texttt{<}, \texttt{>}, \texttt{\&} symbols.
